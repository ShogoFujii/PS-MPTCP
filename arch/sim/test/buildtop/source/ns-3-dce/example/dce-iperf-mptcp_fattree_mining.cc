...
int main (int argc, char *argv[])
{
  ...
  */
  ...
  pareto_make(1.5, 10, num, short_box);
  cout << "num:" << num << endl;
  /*
  for(int i=0; i<num; i++){
  	cout << short_box[i] << endl;
  }
  */
  //rep_which=0; 
  int cnt_flow=0;
  ...
  socketTraffic(long_list, 0, 4.0, end, nodes);

  
  for(int i=0; i<poisson_size;i++){
    //socketTraffic(adrs_set3, s_size, (double) poisson_set[i] / 1000, end, nodes);
    //socketTraffic(adrs_set4, s_size, (double) poisson_set[i] / 1000, end, nodes);
  }
