...
/*
		...
		cout << cmd_oss.str ().c_str () << endl;
        cmd_oss.str ("");
        cmd_oss << "route add default via " << ifc[j*2].GetAddress (1, 0) <<" dev sim" << i <<  " src " << ifc[j*2].GetAddress (0, 0) << " table " << (i+1);
        LinuxStackHelper::RunIp (nodes.Get (j*2), Seconds (0.1), cmd_oss.str ().c_str ());
		cout << cmd_oss.str ().c_str () << endl;
        cmd_oss.str ("");
        cmd_oss << "route add default via " << ifc[j*2].GetAddress (1, 0) << " metric 15";
        LinuxStackHelper::RunIp (nodes.Get (j*2), Seconds (0.1), cmd_oss.str ().c_str ());
		cout << cmd_oss.str ().c_str () << endl;
        cmd_oss.str ("");
...
/*
        ...
        LinuxStackHelper::RunIp (nodes.Get (j*2+1), Seconds (0.1), cmd_oss.str ().c_str ());
        cmd_oss.str ("");
        cmd_oss << "route add default via " << ifc[j*2+1].GetAddress (1, 0) << " dev sim" << i << " src " << ifc[j*2+1].GetAddress (0, 0)<< " table " << (i+1);
		cout << cmd_oss.str ().c_str () << endl;
        LinuxStackHelper::RunIp (nodes.Get (j*2+1), Seconds (0.1), cmd_oss.str ().c_str ());
        cmd_oss.str ("");
        cmd_oss << "route add default via " << ifc[j*2+1].GetAddress (1, 0) << " metric 10";
        LinuxStackHelper::RunIp (nodes.Get (j*2), Seconds (0.1), cmd_oss.str ().c_str ());
		cout << cmd_oss.str ().c_str () << endl;
        cmd_oss.str ("");
...
*/
  ...
  */
  ...
  for (int i=0; i < nRtrs; i++){	
	...
	LinuxStackHelper::RunIp (routers2.Get (i), Seconds (0.1), "addr show");
  }
  ...
  */
  ...
  //adrs_set2.insert(map<int, string>::value_type(2, "10.4.1.1"));
  ...
  int *constant_set = constant_pros(3500, 6000, 20, &constant_size);
  ...
  for(int i=0; i<constant_size;i++){
    ...
    //socketTraffic(adrs_set4, s_size, (double) constant_set[i] / 1000, end, nodes);
    //socketTraffic(adrs_set3, s_size, (double) constant_set[i] / 1000, end, nodes);
  }

  
  for(int i=0; i<poisson_size;i++){
    //socketTraffic(adrs_set3, s_size, (double) poisson_set[i] / 1000, end, nodes);
  }
  
  socketTraffic(adrs_set, 0, 4.0, 6.0, nodes); 
  //socketTraffic(adrs_set3, 70, 4.0, 6.0, nodes); 
  //socketTraffic(adrs_set2, 0, 4.0, 6.0, nodes); 
