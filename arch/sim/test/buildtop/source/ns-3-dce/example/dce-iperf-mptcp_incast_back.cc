...
int main (int argc, char *argv[])
{
  ...
  for (int j=0; j < nodes_num; j++){	
        ...
        //cmd_oss << "route add default via 10." << j+1 << "." << j%2 << ".2 dev sim" << j%2;
		cout << cmd_oss.str ().c_str () << endl;
  ...
  */
  LinuxStackHelper::RunIp (nodes.Get (2), Seconds (0.1), "addr show");
  ...
  */
  ...
  for (int j=1;j < sum_tr; j++){
  	adrs_set.insert(map<int, string>::value_type(j*2, "10.2.1.1"));
	/*
  	if(j%2)
  		adrs_set.insert(map<int, string>::value_type(j*2, "10.4.1.1"));
	else	
  		adrs_set.insert(map<int, string>::value_type(j*2, "10.4.0.1"));
	*/
  }
  ...
  //adrs_set2.insert(map<int, string>::value_type(4, "10.6.0.1"));
  //show_multimap(adrs_set);
  int poisson_size, constant_size;
  //int *poisson_set = poisson_pros(5000, 7000, 100, &poisson_size);
  int *constant_set = constant_pros(3500, 5000, 50, &constant_size);
  ...
  */
  ...
  socketTraffic(adrs_set, 100, 4.0, end, nodes); 
